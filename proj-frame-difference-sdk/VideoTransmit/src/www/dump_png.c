...
int dump_png_to_file(const char *filename, struct video_transmit *ctx) {
	char * title = "vdma framebuffer[0]";
	long buffer_size = ctx->config->width * ctx->config->height * ctx->config->pixel_size;
	unsigned char buffer[buffer_size];

	size_t index = ctx->vdma->current_framebuffer_index;
	memcpy(buffer, ctx->vdma->framebuffers[index].virtual_addr, buffer_size);

	return writeImage(filename, ctx->config->width, ctx->config->height, buffer, title);
}
